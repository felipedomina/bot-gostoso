...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adeus trouxa @${num.split('@')[0]} ja foi tarde👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*FELIPE GOSTOSO* : ${me.name}\n*https://wa.me/81982133322* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*total de contatos bloqueados* : ${blocked.length}\n*O bot está ativo desde 1563* : ${kyun(uptime)}`
    ...
    case 'bot':
		   ...
		   break
    case 'fdp':
		   if (args.length < 1) return reply ('Eu                  não é vc')
		   break
    case 'tmnc':
		   if (args.length < 1) return reply ('vai vc que ja ta acostumado')
		   break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://www.mediafire.com/view/edpmyf4rlureax0`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NOME:* FELIPE GOSTOSO\n*WPP:* wa.me/+5581982133322\n*INSTA:* https://instagram.com/_felima0?igshid=1fjx1t2zrqrsf'})
					break
				case 'seminudes':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://www.mediafire.com/view/quyw5lxpnok4ggd`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*APRECIE COM MODERAÇÃO*'})
					break
				case 'negao':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSjwf0MBIjqH3NWaDmhPKbpx2cvOdao-O-oGQ&usqp=CAU`)
